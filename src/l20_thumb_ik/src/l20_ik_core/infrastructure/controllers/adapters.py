...
from l20_ik_core.paths import DEFAULT_LINKERHAND_SDK_PATH
...
_SEMANTIC_JOINT_ALIASES: dict[str, tuple[str, ...]] = {
    "thumb_dip": ("thumb_ip",),
    "thumb_ip": ("thumb_dip",),
}
...
def _load_mapping_module(sdk_root: str):
    ...
    spec = importlib.util.spec_from_file_location("l20_ik_core_linkerhand_mapping", mapping_path)
...
class LinkerHandModelAdapter:
    ...
    def format_l20_target_qpos(self, qpos: np.ndarray) -> str:
        if self.family != "L20":
            raise ValueError(f"L20 target qpos formatting requires family L20, got {self.family}")
        ...
        return (
            "real target qpos(弧度)\n"
            f"  指根弯曲={self._rounded_joints(['thumb_cmc_pitch', 'index_mcp_pitch', 'middle_mcp_pitch', 'ring_mcp_pitch', 'pinky_mcp_pitch'], values)}\n"
            f"  横摆={self._rounded_joints(['thumb_cmc_roll', 'index_mcp_roll', 'middle_mcp_roll', 'ring_mcp_roll', 'pinky_mcp_roll'], values)}\n"
            f"  横滚={self._rounded_joints(['thumb_cmc_yaw'], values)}\n"
            f"  指中弯曲={self._rounded_joints(['thumb_dip', 'index_pip', 'middle_pip', 'ring_pip', 'pinky_pip'], values)}\n"
            f"  指尖弯曲={self._rounded_joints(['thumb_dip', 'index_dip', 'middle_dip', 'ring_dip', 'pinky_dip'], values)}"
        )
    ...
    def _rounded_joints(self, names: list[str], qpos: np.ndarray) -> list[float]:
        return [round(self._joint(name, qpos), 4) for name in names]
    ...
    def _resolve_joint_name(self, semantic_name: str) -> str:
        ...
        for alias in _SEMANTIC_JOINT_ALIASES.get(semantic_name, ()):
        ...
        for prefix in _PREFERRED_SIDE_PREFIXES.get(self.hand_side, ()):
            ...
            if candidate in self._joint_index:
                return candidate
            for alias in _SEMANTIC_JOINT_ALIASES.get(stripped, ()):
