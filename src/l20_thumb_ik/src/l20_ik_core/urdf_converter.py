...
def _load_rohand_math_module(urdf_path: Path):
    ...
    if module_path is None:
        return None
    spec = importlib.util.spec_from_file_location("l20_ik_core_rohand_math", module_path)
...
def _canonical_hand_frame(hand_side: str) -> np.ndarray:
    from l20_ik_core.acceptance import mirror_pose_to_left, synthetic_hand_pose
    from l20_ik_core.domain.preprocessing import preprocess_landmarks
...
def _semantic_model_point(model, data, *, hand_side: str, name: str, obj_type) -> np.ndarray:
    from l20_ik_core.infrastructure.model_name_resolver import ModelNameResolver
    import mujoco
...
def convert_urdf_to_mjcf(
    ...
    with tempfile.TemporaryDirectory(prefix="l20_ik_core-mujoco-import-") as import_tmp:
        (
            urdf_text,
            _,
            resolved_meshes,
