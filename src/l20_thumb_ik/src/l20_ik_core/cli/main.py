...
def _load_commands():
    try:
        ...
        raise RuntimeError(
            "l20_ik_core CLI optional dependencies are missing. "
            "Install them with `pip install 'l20_ik_core[cli]'`."
        ) from exc
    return commands
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "video":
        ...
        return
    if args.command == "realsense":
        if args.hand == "both":
            raise ValueError("RealSense input currently supports single-hand commands")
        commands._run_realsense(args)
        return
