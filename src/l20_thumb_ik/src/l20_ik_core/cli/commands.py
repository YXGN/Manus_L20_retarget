...
from l20_ik_core.domain import display_hand_side
from l20_ik_core.runtime import (
...
from l20_ik_core.runtime.source_sampling import FixedRateBiHandTrackingSource, FixedRateHandTrackingSource
...
def _timestamped_webcam_video_path(output_dir: str) -> str:
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return str(Path(output_dir) / f"webcam_{timestamp}.mp4")


def _wrap_source_for_raw_webcam_video(source, *, video_output_path: str | None):
    if not video_output_path:
        return source
    return RawInputVideoRecordingSource(source, output_path=video_output_path)
...
def _run_webcam(args: argparse.Namespace) -> None:
    webcam_video_output = (
        _timestamped_webcam_video_path(args.webcam_video_output_dir)
        if args.save_webcam_video
        else None
    )
    source = _wrap_source_for_recording(
        _wrap_source_for_raw_webcam_video(
            MediaPipeInputSource(
                args.camera,
                hand_side=args.hand,
                swap_handedness=args.swap_hands,
                source_desc=f"camera://{args.camera}",
                min_detection_confidence=args.mediapipe_detection_confidence,
                min_tracking_confidence=args.mediapipe_tracking_confidence,
                camera_width=args.camera_width,
                camera_height=args.camera_height,
                camera_fps=args.camera_fps,
                tracking_hold_frames=args.tracking_hold_frames,
                tracking_jump_threshold=args.tracking_jump_threshold,
            ),
            video_output_path=webcam_video_output,
        ),
    ...
    if webcam_video_output is not None:
        extra_lines.append(f"Raw webcam video output: {webcam_video_output}")
    _print_startup(
        engine,
...
def _run_realsense(args: argparse.Namespace) -> None:
    source = _wrap_source_for_recording(
        _wrap_live_hand_source(
            RealSenseMediaPipeInputSource(
                hand_side=args.hand,
                swap_handedness=args.swap_hands,
                source_desc="realsense://color+depth",
                min_detection_confidence=args.mediapipe_detection_confidence,
                min_tracking_confidence=args.mediapipe_tracking_confidence,
                color_width=args.realsense_color_width,
                color_height=args.realsense_color_height,
                depth_width=args.realsense_depth_width,
                depth_height=args.realsense_depth_height,
                fps=args.realsense_fps,
                depth_patch_radius=args.realsense_depth_patch_radius,
    ...
    engine = _build_engine(args, input_type="realsense")
    session = _build_runtime_session(engine, args, visualize=True, show_preview=True)
    _print_startup(
        engine,
        source_desc=source.source_desc,
        ...
        extra_lines=[
            ...
            f"RealSense: color={args.realsense_color_width}x{args.realsense_color_height}"
            f" depth={args.realsense_depth_width}x{args.realsense_depth_height}"
            f" fps={args.realsense_fps}",
            "Depth mode: MediaPipe 2D landmarks + RealSense deprojection",
            "Press 'q' in the camera window to quit.",
        ],
    )
    summary = session.run(source, input_type="realsense")
    _finalize_run(args, summary=summary, source=source)
